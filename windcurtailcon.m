function [c,ceq] = windcurtailcon(x,mpc)
    ...
    pg = x(numOfBuses+1:2*numOfBuses);
    qg = x(2*numOfBuses+1:3*numOfBuses);
    e = x(3*numOfBuses+1:4*numOfBuses);
    f = x(3*numOfBuses+1:4*numOfBuses);
    
    pw = zeros(numOfBuses,1);    
    for i = size(windBusNums,1)
        busIndex = windBusNums(i);
        pw(busIndex) = mpc.gen(mpc.gen(:,1) == busIndex,2);
    end
    ...
    % Power flow balance
    for i = 1:numOfBuses
        ceq = [ceq; G(i,:)*(e(i)*e + f(i)*f) + B(i,:)*(f(i)*e - e(i)*f) - (pg(i) + windfactor(i)*pw(i) - pd(i))/mpc.baseMVA];
